...
def compMove():
    ...
    for key in board.keys():
        if (board[key] == ' '):
            board[key] = bot
            score = dfs_minimax(board, 0, False)
            board[key] = ' '
            if (score > bestScore):
                bestScore = score
                bestMove = key
...
def dfs_minimax(board, depth, maximizing_player):
  """Performs a depth-first search of the minimax game tree.
  ...
  # If we are the maximizing player, find the best possible score by considering
  # all possible moves.
  if maximizing_player:
    ...
    for key in board:
      if board[key] == ' ':
        board[key] = bot
        score = dfs_minimax(board, depth - 1, False)
        board[key] = ' '
        best_score = max(best_score, score)

    return best_score

  # If we are the minimizing player, find the best possible score by considering
  # all possible moves and choosing the one that leads to the worst possible
  # outcome for our opponent.
  else:
    best_score = 800
    for key in board:
      if board[key] == ' ':
        board[key] = player
        score = dfs_minimax(board, depth - 1, True)
        board[key] = ' '
