...
def compMove():
  """Makes a move for the computer player using the A* algorithm.

  Args:
    board: A dictionary representing the current state of the game board.
  """

  # Calculate the f-score for all possible moves.
  f_scores = {}
  for key in board.keys():
    if board[key] == ' ':
      board[key] = bot
      f_score = evaluate_board(board, True) + heuristic(board)
      board[key] = ' '
      f_scores[key] = f_score

  # Choose the move with the lowest f-score.
  best_move = min(f_scores, key=f_scores.get)
